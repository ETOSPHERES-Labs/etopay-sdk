...
impl<'a, T: GetModule> SerdeLayoutBuilder<'a, T> {
    ...
    fn build_data_layout_(
        ...
        let def = match (
            declaring_module.borrow().find_struct_def_by_name(name.as_str()),
            declaring_module.borrow().find_enum_def_by_name(name.as_str()),
        ) {
            (Some((_, struct_def)), None) => Container::Struct(Struct::new(declaring_module.borrow(), struct_def).1),
            (None, Some((_, enum_def))) => Container::Enum(Enum::new(declaring_module.borrow(), enum_def).1),
    ...
    fn build_from_signature_token(
        ...
        Ok(match s {
            ...
            DatatypeInstantiation(inst) => {
                ...
                DatatypeLayoutBuilder::build_from_handle_idx(m, *shi, actual_layouts, resolver, depth + 1)?
                    .into_layout()
            }
