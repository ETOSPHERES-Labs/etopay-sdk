...
use crate::wallet::rebased::{AccountAddress, error::RebasedError, error::Result};

use super::parser_min::NumberFormat;
    ...
    pub fn into_account_address(self, mapping: &impl Fn(&str) -> Option<AccountAddress>) -> Result<AccountAddress> {
        match self {
            Self::Named(n) => mapping(n.as_str())
                .ok_or_else(|| RebasedError::ParsedAddressError(format!("Unbound named address: '{}'", n))),
...
impl NumericalAddress {
    ...
    pub fn parse_str(s: &str) -> Result<NumericalAddress, String> {
        match parse_address_number(s) {
            ...
            {
                Err(RebasedError::ParsedAddressError(format!(
