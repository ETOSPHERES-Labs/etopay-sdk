...
use crate::wallet::rebased::v2::mowe::StructTag;
use crate::wallet::rebased::v2::mowe::TypeTag;

use crate::wallet::rebased::{encoding::Hex, v2::AccountAddress};
...
//crate::base_types::SequenceNumber
use crate::wallet::rebased::v2::iota::iota_types::base_types::SequenceNumber as BaseSequenceNumber;
...
pub static BRIDGE_ADDRESS: LazyLock<AccountAddress> =
    LazyLock::new(|| AccountAddress::from_hex_literal("0xb").expect("Invalid address"));

pub static MOVE_STDLIB_ADDRESS: LazyLock<AccountAddress> =
    LazyLock::new(|| AccountAddress::from_hex_literal("0x1").expect("Invalid address"));

pub static IOTA_FRAMEWORK_ADDRESS: LazyLock<AccountAddress> =
    LazyLock::new(|| AccountAddress::from_hex_literal("0x2").expect("Invalid address"));

pub static IOTA_SYSTEM_ADDRESS: LazyLock<AccountAddress> =
    LazyLock::new(|| AccountAddress::from_hex_literal("0x3").expect("Invalid address"));

// don't know if it's valid, took it from crates/iota-types/src/lib.rs
pub static STARDUST_ADDRESS: LazyLock<AccountAddress> =
    LazyLock::new(|| AccountAddress::from_hex_literal("0x107a").expect("Invalid address"));

pub static IOTA_SYSTEM_STATE_ADDRESS: LazyLock<AccountAddress> =
    LazyLock::new(|| AccountAddress::from_hex_literal("0x5").expect("Invalid address"));

pub static IOTA_CLOCK_ADDRESS: LazyLock<AccountAddress> =
    LazyLock::new(|| AccountAddress::from_hex_literal("0x6").expect("Invalid address"));

pub static IOTA_ADDRESSES: LazyLock<[AccountAddress; 7]> = LazyLock::new(|| {
    [
        AccountAddress::ZERO,
        AccountAddress::ONE,
...
impl SerializeAs<BaseSequenceNumber> for SequenceNumber {
    fn serialize_as<S>(value: &BaseSequenceNumber, serializer: S) -> Result<S::Ok, S::Error>
    where
        S: Serializer,
    {
...
impl<'de> DeserializeAs<'de, BaseSequenceNumber> for SequenceNumber {
    fn deserialize_as<D>(deserializer: D) -> Result<BaseSequenceNumber, D::Error>
    where
        D: Deserializer<'de>,
    {
        let b = BigInt::deserialize(deserializer)?;
        Ok(BaseSequenceNumber::from_u64(*b))
    }
}
