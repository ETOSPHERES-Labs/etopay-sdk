...
use crate::wallet::rebased::v2::mowe::account_address::AccountAddress;
use crate::wallet::rebased::v2::mowe::identifier::Identifier;
use crate::wallet::rebased::v2::mowe::is_valid_identifier_char;
use crate::wallet::rebased::v2::mowe::language_storage::StructTag;
use crate::wallet::rebased::v2::mowe::language_storage::TypeTag;
...
impl Display for TypeToken {
    fn fmt(&self, formatter: &mut fmt::Formatter<'_>) -> std::result::Result<(), std::fmt::Error> {
        let s = match *self {
    ...
    fn next_token(s: &str) -> Result<Option<(Self, usize)>> {
        ...
        Ok(Some(match c {
            ...
            ':' => match chars.next() {
                ...
                _ => return Err(RebasedError::ParserError(format!("unrecognized token: {}", s))),
            },
            ...
            c if c.is_ascii_alphabetic()
                || (c == '_' && chars.peek().is_some_and(|c| is_valid_identifier_char(*c))) =>
            {
                // c + remaining
                let len = 1 + chars.take_while(|c| is_valid_identifier_char(*c)).count();
                (Self::Ident, len)
            }
            _ => return Err(RebasedError::ParserError(format!("unrecognized token: {}", s))),
        }))
